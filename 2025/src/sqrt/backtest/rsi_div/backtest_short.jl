...
function fig_result(initial_capital, returns)
    ...
    plot(
        ...
        xlabel="Period",
        ylabel="Equity (USD)",
        title="Equity Curve (Initial capital = \$$(initial_capital))",
        legend=false,
        linewidth=2
    )
end
...
function rsi(close::Vector{Float64}; length::Int=14, scalar::Float64=100.0, drift::Int=1, offset::Int=0)
...
function check_candle_color(current_ohlcv, prev_ohlcv)
    # 현재 캔들은 음봉 
    if is_bullish(current_ohlcv) == false
        # D-1 캔들은 양봉
        if is_bullish(prev_ohlcv) == true
            return true
        end
    end
    return false
...
# function find_prev_high_rsi(partial_ohlcvs, partial_rsi, min_rsi)
#     count = 0
...
function find_prev_high_rsi(partial_rsi, max_rsi)
    if maximum(partial_rsi) - 5 < max_rsi
        return true
    end
    return false
end
