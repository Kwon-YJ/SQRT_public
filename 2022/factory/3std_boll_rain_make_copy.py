...
def log_maker(ohlcv, ticker_name) -> None:
    interval = len(ohlcv) - 1
    for i in range(100, interval):
        low = ohlcv[i][3]
        ...
        lower_band = bband[0]
        if low < lower_band:
            if ohlcv[i][1] > lower_band:
                entry_price = lower_band
            else:
                entry_price = ohlcv[i][1]

            exit_price = ohlcv[i + 1][4]
            # earning = (exit_price / entry_price * Slippage- 1) * 100
            earning = (entry_price / exit_price * Slippage - 1) * 100
            print(
...
if __name__ == "__main__":
    ...
    temp_time = Utils.timestamp_to_datetime(
        binance.fetch_ohlcv("BTC/USDT", "1d")[-20][0]
    )[
        2
    ]  # 1h 기준 40의 배수
    ...
    for ticker in ticker_list:
        ...
        try:
            ...
            ohlcv = binance.fetch_ohlcv(ticker, "8h", None, 999)
            # ohlcv = binance.fetch_ohlcv(ticker, '15m',timestamp,999)
