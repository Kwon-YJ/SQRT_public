...
def get_target(ticker_name):
    ...
    Ohlcv = Utils.get_ohlcv(ticker_name, timeframe, 100)
    if Ohlcv == None:
        return None
    Ohlcv = [ohlcv[:6] for ohlcv in Ohlcv]
    data = pd.DataFrame(
        data=np.array(Ohlcv), columns=["Time", "Open", "High", "Low", "Close", "V"]
    )  ## rsi(15, high)
    rsi_entry = data.ta.rsi(length=14).tolist()
    ...
    if len(max_price) > 1:
        ...
        # if np.array_equal(max_price,max_rsi) and max_price[1] > 97 and data['rsi'][max_rsi[0]] > 70:
        if (
            ...
            if (
                ...
                before_time = Utils.timestamp_to_datetime(Ohlcv[max_price[0]][0])
                after_time = Utils.timestamp_to_datetime(Ohlcv[max_price[1]][0])

                print(before_time[1], after_time[1])

    if len(min_price) > 1:
        min_before = data.iloc[min_price[0]]
        min_after = data.iloc[min_price[1]]
        # if np.array_equal(min_price,min_rsi) and min_price[1] > 97 and data['rsi'][min_rsi[0]] < 30:
        if (
            ...
            if (
                ...
                before_time = Utils.timestamp_to_datetime(Ohlcv[min_price[0]][0])
                after_time = Utils.timestamp_to_datetime(Ohlcv[min_price[1]][0])
...
if __name__ == "__main__":
    ...
    for ticker in ticker_list:
        # get_target(ticker)
        result = parmap.map(
            get_target, ticker_list, pm_pbar=False, pm_processes=num_cores
        )

    timeframe = "3m"
    print(timeframe)
    for ticker in ticker_list:
        # get_target(ticker, '3m')
        result = parmap.map(
            get_target, ticker_list, pm_pbar=False, pm_processes=num_cores
        )
