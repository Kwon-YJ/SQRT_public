...
while 1:
    try:
        time.sleep(11)
        if int(get_time()[1]) % 15 != 0:
            time.sleep(1)
            continue

        _15m_data = get_HA_ohlcv(ticker, "15m")  # 15분 전

        price = binance.fetch_ohlcv(ticker, "5m")[-1][4]
        if _15m_data[-2][1] < _15m_data[-2][4] and _15m_data[-1][1] > _15m_data[-1][4]:
            if len(prices) == 0:
                prices.append(price)
                time.sleep(60)
                continue
            earning = round(-100 + prices[-1] / price * 99.985, 2)
            messege = ["buy switch", price, earning]
        ...
        if _15m_data[-2][1] > _15m_data[-2][4] and _15m_data[-1][1] < _15m_data[-1][4]:
            if len(prices) == 0:
                ...
                time.sleep(60)
                continue
            earning = round(-100 + price / prices[-1] * 99.985, 2)
            messege = ["sell switch", price, earning]
            bot.send_message(chat_id=801167350, text=str(messege))
            prices.append(price)
