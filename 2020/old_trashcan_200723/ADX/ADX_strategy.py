...
def _entry(ohlcv):
    ...
    if ohlcv[-2][4] > ohlcv[-6][4]:
        order = binance.create_order(ticker, "market", "buy", 3, None, {"leverage": 1})
        status_[0] = 1
        status_[1] = 0

    elif ohlcv[-2][4] < ohlcv[-6][4]:
        order = binance.create_order(ticker, "market", "sell", 3, None, {"leverage": 1})
        status_[0] = 1
        status_[1] = 1
...
def _exit(ticker, ohlcv):
    if status_[1] == 0:
        ...
        return None
    if temp_[-1] == point:
        order = binance.create_order(ticker, "market", side, 3, None, {"leverage": 1})
        status_[0] = 0
        status_[1] = -1
