...
def get_funding_rate(ticker):
    ...
    for i in range(EndPoint):
        All_funding_rate[tickers[i]["symbol"]] = abs(
            float(tickers[i]["lastFundingRate"])
        )
...
def get_amount(ticker):
    ...
    result = round(today_money_USDT / price * 0.017, decimal_amount)
    return result
...
def buy_order(ticker, sell_price):
    ...
    is_entering[ticker] = [float(order["amount"]), sell_price]
...
while True:
    ...
    if time_ == "00":
        for i, item in enumerate(ticker_list):
            try:
                if get_funding_rate(item) > 0.0001:
                    continue
