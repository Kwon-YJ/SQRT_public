...
futuers_tickers = ["YFI/USDT", "YFII/USDT", "LINK/USDT", "THETA/USDT"]
...
while 1:
    ...
    if (
        ...
        and last_position[1] == None
        and last_position[2] == None
        and last_position[3] == None
    ):
        break

while 1:
    for i, item in enumerate(futuers_tickers):
        if last_position[i] == None and check_match(item) == "buy":
            amount = get_amount(item)
            order = binance.create_order(item, "market", "buy", amount)
            position_size[i] = float(order["amount"])
        if last_position[i] == None and check_match(item) == "sell":
            amount = get_amount(item)
            order = binance.create_order(item, "market", "sell", amount)
            position_size[i] = float(order["amount"])
        if last_position[i] == "sell" and check_match(item) == None:
            amount = position_size[i]
            order = binance.create_order(item, "market", "buy", amount)
            position_size[i] = 0
        if last_position[i] == "buy" and check_match(item) == None:
            amount = position_size[i]
            order = binance.create_order(item, "market", "buy", amount)
            position_size[i] = 0
