...
def log_maker():
    for j in range(len(ticker_list)):
        ...
        for i in range(2, len(ohlcv) - 1):
            TR_value = ohlcv[i - 1][2] - ohlcv[i - 1][3]
            if ohlcv[i][2] > ohlcv[i][1] + TR_value * 0.7:
                buy_price = ohlcv[i][1] + TR_value * 0.7
                sell_price = ohlcv[i][4]
                earning = 100 * ((sell_price / buy_price) * 0.99925 * 0.99925 - 1)
