...
def log_maker(ticker):
    ...
    for i in range(17, len(df) - 2):
        ...
        if HH_price < now_close and len(price) == 0:
            HH_price = now_close
            if len(price) == 0:
        ...
        if len(price) != 0:
            ...
            if now_close < df.iloc[i]["종가"]:
                temp += 1
            elif now_close > df.iloc[i]["종가"]:
                temp -= 1

            if temp == 10:
                earning = 100 * (
                    (df.iloc[i + 1]["시가"] / price[-1]) * 0.99925 * 0.99925 - 1
                )
            ...
            temp = get_ATR(15, now_ohlcv)
            if now_close < HH_price - (2.7 * temp) or i == len(df)-2:
                tomorrow_open = df.iloc[i]['시가']
                if tomorrow_open == 0:
                    continue
...
if __name__ == "__main__":
    ...
    All_tickers.sort()
    All_tickers = All_tickers[: int(len(All_tickers) / 10)]
    a = time.time()
    result_ = sum(parmap.map(log_maker, All_tickers, pm_pbar=True, pm_processes=16), [])
    b = time.time()
    print(b - a)
