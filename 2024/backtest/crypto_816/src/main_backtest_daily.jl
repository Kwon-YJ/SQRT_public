...
function back_test(open_data, high_data, low_data, close_data, time_data,ema_8,ema_16,tail_d,body_d, param_1::Float64, param_2::Float64, ticker_name::String, time_frame::String)
        ...
        if is_long == false
            if low_ < short_ema && min(close_, open_) > short_ema && short_ema > ema_16[i]
                if body_d[i] * param_1 < tail_d[i]
                    ...
                    long_entry_time = "$(Dates.unix2datetime(time_data[i+1] / 1000))"[1:10]
                end
            end
        else
            if low_ < long_sl[end]
                ...
                exit_time = "$(Dates.unix2datetime(time_data[i] / 1000))"[1:10]
                df = DataFrame(ticker = ticker_name, exit = exit_time, earn = earning)
                CSV.write("../daily_result/$(time_frame)/$(long_entry_time).csv", df, append=true)
                continue
...
function load_params(data_dir="../../data/", file_name="E6_1d.CSV")
    csv_data = CSV.read(data_dir*file_name, DataFrame)
    column_names = string.(names(csv_data))
    return column_names
end
...
for time_frame in time_frames
    data_dir = "../data/$(time_frame)/"

    mkdir_exist_ok("../daily_result/")
    mkdir_exist_ok("../daily_result/$(time_frame)")
    ...
    for file_name = readdir(data_dir)
        ...
        param_2 = 0
        try
            param_1, param_2 = load_params("../result/$(time_frame)/", file_name)[1:2]
        catch
            println("err in line 164")
            continue
        end

        param_1 = replace(param_1, "_1" => "")
        param_2 = replace(param_2, "_1" => "")

        param_1 = parse(Float64, param_1)
        param_2 = parse(Float64, param_2)


        back_test(open_data, high_data, low_data, close_data, time_data,ema_8,ema_16,tail_d,body_d , param_1, param_2, file_name, time_frame)
        # try
