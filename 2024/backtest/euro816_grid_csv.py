...
def load_data(data_dir="../../data/", file_name="E6_1d.CSV"):
    ...
    try:
        csv_data.columns = ["t", "o", "h", "l", "c", "v", "s", "real_v"]
    except:
        csv_data.columns = ["t", "o", "h", "l", "c", "v"]
...
def back_test(param_1=3, param_2=1.5):
    ...
    for i in range(1, len(ohlc) - 1):
        ...
        if is_short == False:
            ...
            if high_ > short_ema and max(close_, open_) < short_ema and short_ema < long_ema: 
                ...
                if body_d * param_1 < tail_d:
                    ...
                    short_tp.append(open_data[i+1] - param_2 * abs((high_-min(close_, open_)))) 
    ...
    result = get_perfomance(trade_log)
    # result = total_perform.money_simulation()

    csv_init([param_1, param_2, len(trade_log), result], args.file_name)


def csv_init(input_data, file_name):
    with open(f"./{file_name}_816.csv", "a", newline="") as file:
        ...
        # param_1, param_2, 거래 횟수, 거리


param_1_group = np.linspace(0.8, 5, 35).tolist()

param_2_group = np.linspace(0.8, 5, 35).tolist()

global args
args = make_parser().parse_args()


csv_init([], file_name=args.file_name)
...
# ohlc, ema_8, ema_16 = load_data(file_name="E6_5m.CSV")
# ohlc, ema_8, ema_16 = load_data(file_name="E6_1d.CSV")


ohlc, ema_8, ema_16 = load_data("fx_data/", file_name=args.file_name)

for param_1 in param_1_group:
    for param_2 in param_2_group:
        back_test(param_1=param_1, param_2=param_2)
