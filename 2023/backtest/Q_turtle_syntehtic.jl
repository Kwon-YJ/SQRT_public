    ...
    for day::Int64 in limit-2:-1:0
        ...
        @inbounds for (ticker1::String, ticker2::String) in ticker_list
            ...
            if low_trade < entry_price
                ...
                #if All_ohlcv[ticker1][end-day][1][2] > All_ohlcv[ticker1][end-day][idx][2]
                #    continue
                #end

                #if All_ohlcv[ticker2][end-day][1][2] < All_ohlcv[ticker2][end-day][idx][2]
                #    continue
                #end

                #temp_entry_price = All_ohlcv[ticker1][end-day][idx][2]
                #temp_exit_price = All_ohlcv[ticker1][end-day][end][2]
                #temp_earning = -100(temp_exit_price / temp_entry_price * slippage - 1)

                #push!(total_buy_sell_log, temp_earning)
                #push!(buy_sell_log, temp_earning)
                ...
                push!(total_buy_sell_log, long_earning)
                push!(total_buy_sell_log, short_earning)
                push!(buy_sell_log, long_earning)
                push!(buy_sell_log, short_earning)
