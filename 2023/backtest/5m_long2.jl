...
function log_maker(limit, All_ohlcv, ticker_list)
    ...
    for (i, ohlcv) in enumerate(All_ohlcv)
        ...
        # tr2 = (Moving_average(True_range(ohlcv), 4) .* 2)[end-1439:end]
        # tr2 = (Moving_average(True_range(ohlcv), 4) .* 4)[end-1439:end]
        tr2 = (Moving_average(True_range(ohlcv), 4) .* 5)[end-1439:end]
        ...
        for j=length(rsi_)-2:-1:1
            if (ohlcv[end-j][2] + ohlcv[end-j][5]) / 2 > entry_target[end-j] && (ohlcv[end-j-1][2] + ohlcv[end-j-1][5]) / 2 < entry_target[end-j-1] && length(long_entry_price) == 0
    ...
    total_perform, trade_count = get_performance(buy_sell_log, true)
    result_dict[string(count)] = total_perform
    empty!(buy_sell_log)
...
for i in range(1, 295)
    ...
    println("$(unix2datetime(All_ohlcv[1][1][1]/1000))")
    println("$(unix2datetime(All_ohlcv[1][end][1]/1000))")
