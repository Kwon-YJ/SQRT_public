...
function log_maker(ohlcv)
    ...
    rsi_ = rsi(low, 14)[end-(length(close)-25):end]
    exit_target = ma - tr2
    ...
    for j=length(rsi_)-2:-1:2
        if rsi_[end-j-1] < 19.5 && length(long_entry_price) == 0 && ohlcv[end-j][2] > ohlcv[end-j+1][2]
            push!(long_entry_price, ohlcv[end-j+1][2])
            push!(long_entry_time_buffer, ohlcv[end-j+1][1])
            continue
        end
        if length(long_entry_price) != 0
            if exit_target[end-j] > (ohlcv[end-j][2] + ohlcv[end-j+1][2]) / 2 && exit_target[end-j-1] < (ohlcv[end-j-1][2] + ohlcv[end-j][2]) / 2
