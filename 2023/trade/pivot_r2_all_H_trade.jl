...
function entry_side(data, current_price, double_ohlcv)
    ...
    for ticker in ticker_list
        ...
        if current_price[ticker] > target_price && fetch_available_balance() > 200
            trade_amount, price = get_amount(ticker, 1240)
            order_resp = create_order(ticker, "SELL", "MARKET", trade_amount, price)
            data.order_dict[ticker] = order_resp["origQty"]
            telegram_send("SELL entry (R2) : $(ticker) // $(data.time_frame)h // $(now(Dates.UTC))")
        end
    end
end
...
function exit_side(data)
    ...
    if hours%data.time_frame == 0 && minute == 0
        for ticker in keys(data.order_dict)
            ...
            telegram_send("BUY exit (S2) : $(ticker) // $(data.time_frame)h // $(utcurrentime)")
        end
...
function main()
    ...
    _8h_data = data_struct(get_target_price("8h"), Dict(), 0.0025, 2.25, 2.25, 8)
...
del_list = []
for ticker in ticker_list
    if length(fetch_ohlcv(ticker, "1d", 5)) != 5
