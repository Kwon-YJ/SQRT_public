    ...
    for i in range(2, len(ema_25) - 1):
        ...
        if price == 0:
            ...
            if (
                ...
                and ohlcv_temp[i - 1][4] > ema_25[i - 1]
            ):
...
def get_ohlcv(ticker="BTC/USDT", interval="1m", limit=200):
    try:
        ...
        # endTime = str(get_ohlcv('BTC/USDT')[-1][0])

        # url = f'https://fapi.binance.com/fapi/v1/klines?symbol={ticker}&interval={interval}&limit={str(limit)}' # monitoring
        url = f"https://fapi.binance.com/fapi/v1/klines?symbol={ticker}&interval={interval}&startTime={startTime}&endTime={endTime}&limit={str(limit)}"  # backtesting

        # url = f'https://api.binance.com/api/v3/klines?symbol={ticker}&interval={interval}&limit={str(limit)}' # backtesting(spot)
