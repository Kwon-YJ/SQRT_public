...
def get_ohlcv(ticker="BTC/USDT", interval="1m", limit=200):
    try:
        ...
        url = f"https://api.binance.com/api/v3/klines?symbol={ticker}&interval={interval}&endTime={endTime}&limit={str(limit)}"  # backtesting(spot)
...
for day in range(2, 21):
    ...
    for i in range(len(All_ohlcv)):
        ...
        ema_100 = (df["close"].ewm(99).mean()).tolist()  # 100

        # log_maker(all_tickers_final[i], ohlcv_temp[-97:],ema_25[-97:], ema_50[-97:], ema_100[-97:])
        log_maker(
            all_tickers_final[i],
            ohlcv_temp[-865:],
            ema_25[-865:],
