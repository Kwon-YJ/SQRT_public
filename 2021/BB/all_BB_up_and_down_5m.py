...
def bb(x, w=20, k=2):  # 볼린저 밴드
    ...
    return [lbb.tolist()[-9:], mbb.tolist()[-9:], ubb.tolist()[-9:]]
...
def BB_checking(ohlcv15, name):
    ...
    ohlcv15 = ohlcv15[-9:]
    if ohlcv15[-1][4] > BB_value[2][-1] and ohlcv15[-2][4] < BB_value[2][-2]:
        temp = [i for i in range(len(ohlcv15)) if ohlcv15[i][4] < BB_value[0][i]]
        if len(temp) != 0:
            # msg ='5min  ' + name + '  sell entry'
            # send_MSG(msg)
            # great_escape(1, name)
            result.append(name)

    elif ohlcv15[-1][4] < BB_value[0][-1] and ohlcv15[-2][4] > BB_value[2][-2]:
        temp = [i for i in range(len(ohlcv15)) if ohlcv15[i][4] > BB_value[2][i]]
        if len(temp) != 0:
            # msg ='5min  ' + name + '  buy entry'
            # send_MSG(msg)
            # great_escape(-1, name)
            result.append(name)
