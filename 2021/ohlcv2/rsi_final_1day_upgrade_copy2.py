...
for j in range(199, 2, -1):
    ...
    file_name2 = f"{day-1}.pickle"

    with open(file_name2, "rb") as fr:
        All_ohlcv_D_1 = pickle.load(fr)

    for i in range(len(All_ohlcv)):
        ohlcv_temp = All_ohlcv[i]

        ohlcv_D_1 = All_ohlcv_D_1[i]

        # if ohlcv_D_1[0][1] > ohlcv_D_1[-1][4]: # 전일 양봉
        if ohlcv_D_1[0][1] < ohlcv_D_1[-1][4]:  # 전일 음봉
            continue
