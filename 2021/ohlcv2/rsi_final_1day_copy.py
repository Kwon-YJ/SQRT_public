...
def log_maker(ohlcv_temp, rsi_entry, rsi_exit):
    price = 0
    entry_time_buffer = []
    ...
    for i in range(len(rsi_entry) - 1):
        ...
        if price == 0:
            ...
            # if rsi_entry[i-1] < number and ohlcv_temp[i][1] > ohlcv_temp[i][4] and ohlcv_temp[i][5] > ohlcv_temp[i-1][5] and ohlcv_temp[i][5] > ohlcv_temp[i-2][5]:

            # abs(ohlcv_temp[i][1] - ohlcv_temp[i][4]) > abs(ohlcv_temp[i-1][1] - ohlcv_temp[i-1][4])

            # if rsi_entry[i-1] < 21 and ohlcv_temp[i][1] > ohlcv_temp[i][4] and ohlcv_temp[i][5] > ohlcv_temp[i-1][5] and ohlcv_temp[i][5] > ohlcv_temp[i-2][5] and abs(ohlcv_temp[i][1] - ohlcv_temp[i][4]) > abs(ohlcv_temp[i-1][1] - ohlcv_temp[i-1][4]):
            # if rsi_entry[i-1] < 21 and ohlcv_temp[i][1] > ohlcv_temp[i][4] and ohlcv_temp[i][5]*1.5 > ohlcv_temp[i-1][5] and ohlcv_temp[i][5]*1.5 > ohlcv_temp[i-2][5] and abs(ohlcv_temp[i][1] - ohlcv_temp[i][4]) > abs(ohlcv_temp[i-1][1] - ohlcv_temp[i-1][4]):
            if (
                rsi_entry[i - 1] < 21
            ...
            if rsi_exit[i] > 45:
                ...
                # print(entry_time_buffer[0][:5] + '  buy : '+entry_time_buffer[0][6:] +'  // sell : '+exit_time +'  '+ str(earning))
...
def get_perfomance(trade_log):
    ...
    if total_perform > 0 and _123[0] == 0:
        _123[0] = +1
    elif total_perform > 0 and _123[0] != -1:
        _123[0] = +1
        return None
    elif total_perform < 0 and _123[0] != -1:
        _123[0] = -1
...
for j in range(198, 2, -1):
    # for j in range(90, 50, -1):
    print(j, "일 전 부터", j - 1, "일 전 까지")

    day = j

    file_name = f"{day}.pickle"
    ...
    for i in range(len(All_ohlcv)):
        ...
        rsi_exit = df.ta.rsi(length=9).tolist()
        # log_maker(all_tickers_final[i], ohlcv_temp[70:166],rsi_entry[70:166], rsi_exit[70:166])
        log_maker(ohlcv_temp[70:166], rsi_entry[70:166], rsi_exit[70:166])

    get_perfomance(buy_sell_log)

    get_perfomance2(temp_temp)
