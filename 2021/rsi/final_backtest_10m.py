...
# ccxt 전용 5분봉 2개를 합쳐 10분봉으로 바꿔주는 함수
def custom_convert(ohlcv5):  # convert 5m → 20m
    ...
    #     del ohlcv5[i]
    ohlcv10 = []
    for i in range(0, len(ohlcv5) - 1, 2):
        highs = [ohlcv5[i + j][2] for j in range(0, 2) if ohlcv5[i + j][2]]
        lows = [ohlcv5[i + j][3] for j in range(0, 2) if ohlcv5[i + j][3]]
        candle = [
            ohlcv5[i + 0][0],
            ohlcv5[i + 0][1],
...
def log_maker(name_, ohlcv_temp, rsi_entry, rsi_exit):
    ...
    for i in range(2, len(rsi_entry) - 1):
        if (
            ...
            continue
        if price == 0:
            # if rsi_entry[i] < 19 and ohlcv_temp[i][5] > ohlcv_temp[i-1][5] and ohlcv_temp[i][5] > ohlcv_temp[i-2][5] and rsi_entry[i-1] > 19:
            # if rsi_entry[i] < 19 and ohlcv_temp[i][5] > ohlcv_temp[i-1][5] * 2 and ohlcv_temp[i][5] > ohlcv_temp[i-2][5] * 2 and rsi_entry[i-1] > 19:
            # if rsi_entry[i] < 19 and ohlcv_temp[i][5] > ohlcv_temp[i-1][5] * 1.3 and ohlcv_temp[i][5] > ohlcv_temp[i-2][5] * 1.3  and rsi_exit[i] < 25:
            # if rsi_entry[i] < 19 and ohlcv_temp[i][5] * 2 > ohlcv_temp[i-1][5] + ohlcv_temp[i-2][5] and rsi_exit[i] < 25:
            # if rsi_entry[i] < 18.5 and ohlcv_temp[i][5] > ohlcv_temp[i-1][5] and ohlcv_temp[i][5] > ohlcv_temp[i-2][5]:
            # if (rsi_entry[i] + rsi_entry[i-1]) / 2  < 23.5:
            # if rsi_entry[i] < 25 and ohlcv_temp[i][5] > ohlcv_temp[i-1][5] * 2 and ohlcv_temp[i][5] > ohlcv_temp[i-2][5] * 2 and rsi_entry[i-1] > 30:
            # if rsi_entry[i-1] < 25 and ohlcv_temp[i][1] > ohlcv_temp[i][4]:
            ...
            # if rsi_entry[i-2] < 25 and ohlcv_temp[i-1][1] > ohlcv_temp[i-1][4] and ohlcv_temp[i][1] > ohlcv_temp[i][4]:
            # if rsi_entry[i-2] < 25 and ohlcv_temp[i-1][1] > ohlcv_temp[i-1][4] and ohlcv_temp[i][1] > ohlcv_temp[i][4] and ohlcv_temp[i][5] > ohlcv_temp[i-1][5] and ohlcv_temp[i][5] > ohlcv_temp[i-2][5]:
            ...
            # if rsi_entry[i-1] < 23.5 and ohlcv_temp[i][1] > ohlcv_temp[i][4] and ohlcv_temp[i][5] > ohlcv_temp[i-1][5] and ohlcv_temp[i][5] > ohlcv_temp[i-2][5] and rsi_entry[i-2] > 23.5:
            # if rsi_entry[i-1] < 27 and ohlcv_temp[i][1] > ohlcv_temp[i][4] and ohlcv_temp[i][5] > ohlcv_temp[i-1][5] and ohlcv_temp[i][5] > ohlcv_temp[i-2][5] and rsi_entry[i-2] > 27:
            # if rsi_entry[i-1] < 23.5 and ohlcv_temp[i][1] > ohlcv_temp[i][4]:
            # if rsi_entry[i-1] < 25 and ohlcv_temp[i][1] > ohlcv_temp[i][4] and ohlcv_temp[i][5] > ohlcv_temp[i-1][5] and ohlcv_temp[i][5] > ohlcv_temp[i-2][5] and rsi_entry[i-2] > 25: ## the best
            # if rsi_entry[i-1] < 25 and ohlcv_temp[i][1] > ohlcv_temp[i][4] and ohlcv_temp[i][5] > ohlcv_temp[i-1][5] and ohlcv_temp[i][5] > ohlcv_temp[i-2][5]:
            # if rsi_entry[i-1] < 17.5:
            # if rsi_entry[i] < 17.5:
            # if rsi_entry[i] < 17.5:
            if rsi_entry[i] < 15:
            ...
            if rsi_exit[i] > 39:
                ...
                # name_save.append('0')
                # print(name_ +'  ' +entry_time_buffer[0][:5] + '  buy : '+entry_time_buffer[0][6:] +'  // sell : '+exit_time +'  '+ str(earning))
...
for day in range(2, 20):
    ...
    for i in range(len(ticker_list)):
        try:
            ...
            temp = custom_convert(get_ohlcv(ticker_list[i], time_frame, 1500))
    ...
    for i in range(len(All_ohlcv)):
        ...
        log_maker(
            all_tickers_final[i], ohlcv_temp[-576:], rsi_entry[-576:], rsi_exit[-576:]
        )
        ...
        # log_maker(all_tickers_final[i], ohlcv_temp[-289:],rsi_entry[-289:], rsi_exit[-289:])
