...
def log_maker(name_, ohlcv_temp, rsi_entry, rsi_exit):
    ...
    for i in range(4, len(rsi_entry) - 1):
        ...
        if price == 0:
            if rsi_entry[i] < 17.5 and rsi_entry[i - 1] > 30:
...
for day in range(16, 21):
    ...
    for i in range(len(ticker_list)):
        try:
            ...
            # endTime = int(binance.fetch_ohlcv('BTC/USDT','1d')[-day*multiple +1][0])
            # endTime = int(binance.fetch_ohlcv('BTC/USDT','1d')[(-day+1)*multiple-1][0])
            endTime = int(
                binance.fetch_ohlcv("BTC/USDT", "1d")[(-day + 1) * multiple][0]
            )
