...
def log_maker(name_, ohlcv_temp):
    price = 0
    active_price = 0
    entry_time_buffer = []

    for i in range(2, len(ohlcv_temp) - 1):
        if str(ohlcv_temp[i]) == "nan" or ohlcv_temp[i] == None:
            continue
        if price == 0:
            if ohlcv_temp[i][1] / ohlcv_temp[i][4] > 1.02:
                price = ohlcv_temp[i + 1][1]
                active_price = ohlcv_temp[i + 1][1]
                entry_time_buffer.append(timestamp_to_datetime(ohlcv_temp[i + 1][0])[1])
                continue
        else:

            if ohlcv_temp[i - 1][2] > active_price:
                active_price = ohlcv_temp[i - 1][2]

            if active_price > ohlcv_temp[i][1]:
                earning = 100 * (ohlcv_temp[i][1] * 0.98 / price * Slippage - 1)
            ...
            if active_price * 0.98 > ohlcv_temp[i - 1][2]:
                earning = 100 * (active_price * 0.98 / price * Slippage - 1)
                buy_sell_log.append(earning)
                temp_temp.append(earning)
                exit_time = timestamp_to_datetime(ohlcv_temp[i + 1][0])[0]
                # name_save.append('0')
...
for day in range(2, 50):
    ...
    for i in range(len(ticker_list)):
        try:
            temp = get_ohlcv(ticker_list[i], time_frame, 1500)
    ...
    for i in range(len(All_ohlcv)):
        ...
        log_maker(all_tickers_final[i], ohlcv_temp[-1440:])
