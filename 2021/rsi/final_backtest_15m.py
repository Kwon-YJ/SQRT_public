...
def get_ohlcv(ticker="BTC/USDT", interval="1m", limit=200):
    ...
    try:
        ...
        url = f"https://fapi.binance.com/fapi/v1/klines?symbol={ticker}&interval={interval}&endTime={endTime}&limit={str(limit)}&startTime={startTime}"  # backtesting
...
for day in range(21, 41):
    ...
    for i in range(len(ticker_list)):
        try:
            temp = get_ohlcv(ticker_list[i], time_frame, 96)
    ...
    for i in range(len(All_ohlcv)):
        ...
        log_maker(
            all_tickers_final[i], ohlcv_temp[-97:], rsi_entry[-97:], rsi_exit[-97:]
        )
        # log_maker(all_tickers_final[i], ohlcv_temp[-385:],rsi_entry[-385:], rsi_exit[-385:])
