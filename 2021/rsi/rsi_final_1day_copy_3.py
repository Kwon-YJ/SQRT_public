...
def log_maker(name_, ohlcv_temp, rsi_entry, rsi_exit):
    ...
    for i in range(4, len(rsi_entry) - 1):
        if (
            str(rsi_entry[i]) == "nan"
        ...
        if price == 0:
            if rsi_entry[i] < 17.5:  # and rsi_entry[i-1] > 25:
...
# 거래량 기반 차트 생성
def get_volume_based_chart(target_ohlcv, ticker_name):
    ...
    temp_time = binance.fetch_ohlcv("BTC/USDT", "1d")
    startTime = int(temp_time[-day * multiple - 1][0])
    endTime = int(temp_time[(-day + 1) * multiple - 1][0])

    temp = get_ohlcv(ticker_name, time_frame, 1000)
    temp = [temp[i][5] for i in range(len(temp))]
    # V_threshold = max(temp) * 1.5
    V_threshold = sum(temp) / len(temp) * 10
    # print(V_threshold)
    ...
    while True:
        try:
            V_value = 0
            lenght_ = 0
            for lenght_ in range(1, len(target_V) + 1):
...
for day in range(2, 366):
    ...
    temp_time = binance.fetch_ohlcv("BTC/USDT", "1d")
    startTime = int(temp_time[-day * multiple][0])
    endTime = int(temp_time[(-day + 1) * multiple][0])

    # for i in range(len(ticker_list)):
    for i, item in enumerate(ticker_list):
        try:
            temp = get_ohlcv(item, time_frame, 1000)
            temp = get_volume_based_chart(temp, item)
            all_tickers_final.append(item)
    ...
    for i, item in enumerate(All_ohlcv):
