...
def log_maker():
    ...
    for i in range(len(ticker_list)):
        temp = get_ohlcv(ticker_list[i], time_frame, 194)
        if len(temp) != 194:
            print(ticker_list[i])
            continue
    ...
    for magic_number in range(1, 192):
        a = {}
        b = {}
        c = {}
    ...
    for i in range(1, len(buy_sell_log)):
        ...
        if buy_sell_log[i] != None:
            ...
            # print(buy_sell_log[i], timestamp_to_datetime(base_ohlcv[i+1][0])[1], round(earning, 2), entry_price, exit_price)
