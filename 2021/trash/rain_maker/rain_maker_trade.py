...
def get_ticker_list(target="/USDT"):  # use only '/USDT', '/BTC', '/ETH', '/BNB'
    ...
    dropout_delisting = parmap.map(
        discard_delisting, temp_list, pm_pbar=True, pm_processes=2
    )  # 상폐 종목 제외 multi
    dropout_delisting = list(
        set(dropout_delisting) - set([None])
    )  # 싱글 케이스는 이 곳을 주석 처리
...
# ticker_list = get_ticker_list('/USDT')
...
for item in ticker_list:
    ...
    ohlcv = get_ohlcv(item, time_frame, 5)
    open_ = ohlcv[-2][1]
    high_ = ohlcv[-2][2]
    low_ = ohlcv[-2][3]
    close_ = ohlcv[-2][4]
...
print(datetime.datetime.now() - aaa)
