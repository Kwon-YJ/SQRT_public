...
# ticker_list = ['BTC/USDT', 'ETH/USDT', 'BCH/USDT', 'LTC/USDT','LINK/USDT','ADA/USDT', 'BNB/USDT', 'DOT/USDT']
    ...
    while 1:
        ...
        if temp == 0:
            ...
            for i, item in enumerate(ticker_list):
                ...
                RSI = calcRSI(ohlcv15_, 15)
                up, down = 0, 0
                temp = RSI.tail(15)
                last = float(RSI.tail(2).iloc[1])
                for i in range(len(temp)):
                    value = float(temp.iloc[i])
                    if value < 30:
                        down += 1
                    if value > 70:
                        up += 1
                if up > 2 and last > 70 or down > 2 and last < 30:
                    msg = item + "\n" + str(up) + "  " + str(down)
                    send_MSG(msg)
            time.sleep(60)
