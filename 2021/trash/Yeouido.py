...
def get_perfomance(trade_log):
    ...
    # print('퍼포먼스 :', shortest_distance(-1 * np.mean(win) / np.mean(lose), len(win) / len(trade_log)))
    # print('포지션 사이징 :', size)
    if len(lose) > 0:
        # print(lose)
    ...
    # plt_data = [손익비, 승률]
    # plt_date.append([len(win) / len(trade_log), -1 * np.mean(win) / np.mean(lose)])
    plt_date.append([-1 * np.mean(win) / np.mean(lose), len(win) / len(trade_log)])
...
def get_ticker_list():
    ...
    std_time_data = binance.fetch_ohlcv("BTC/KRW", "1d")[-1][0]
    result_ = []
    # result_ = [temp_list[i] for i in range(len(temp_list)) if binance.fetch_ohlcv(temp_list[i], '1d')[-1][0] == std_time_data and len(binance.fetch_ohlcv(temp_list[i], '1d')) > 200] # 상폐 종목 제외, 데이터 200일 이상 보유
    for i in range(len(temp_list)):
        time.sleep(2)
        if (
            binance.fetch_ohlcv(temp_list[i], "1d")[-1][0] == std_time_data
            and len(binance.fetch_ohlcv(temp_list[i], "1d")) > 50
        ):
...
for idx, item in enumerate(plt_date):
