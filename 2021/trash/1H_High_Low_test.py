        ...
        while 1:
            try:
                if len(result) != 2:
                    ...
                    break
                # if (result[0][1] > 1.5 and result[1][1] > 1) or (result[0][1] > 1 and result[1][1] > 1.5):
                # if (result[0][1] > 1.8 and result[1][1] > 1.8):
                if result[1][1] > 2.2:
                    # if 1 > 0:
                    # bot.send_message(chat_id = 801167350, text = str(result))
                    # short_ = binance.fetch_ohlcv(result[0][0], '1h')
                    long_ = binance.fetch_ohlcv(result[1][0], "1h")
                    # a = 100 * ((short_[-1-hour_][1] / short_[-1-hour_][4]) * 0.99925 * 0.99925 - 1)
                    b = 100 * (
                        (long_[-1 - hour_][1] / long_[-1 - hour_][4])
                        * 0.99925
                        * 0.99925
                        - 1
                    )
                    # print(i)
                    # print(a, result[0][0], b, result[1][0])
                    # final_.append(a)
                    final_.append(b)
            except:
                time.sleep(2)
