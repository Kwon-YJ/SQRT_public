...
def get_random_ticker():
    ...
    rand_ticker_1 = 0
    rand_ticker_2 = 0
    while rand_ticker_1 == rand_ticker_2:  # 중복 피하기
        rand_ticker_1 = tickers[randint(0, len(tickers) - 1)]
        rand_ticker_2 = tickers[randint(0, len(tickers) - 1)]
    return rand_ticker_1, rand_ticker_2
...
while True:
    ...
    while True:
        ...
        if len(is_entering) == 0:
            ticker_1, ticker_2 = get_random_ticker()
            entry_order(ticker_1, "buy")
            entry_order(ticker_2, "sell")

        # if today_money_USDT * 1.012 < float(binance.fetch_balance()['info']['assets'][0]['walletBalance']):
        if today_money_USDT * 1.012 < float(
